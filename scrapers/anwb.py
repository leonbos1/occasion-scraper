...
BASE_URL = 'https://www.anwb.nl/auto/kopen/zoeken'
...
def scrape_blueprint(driver: webdriver, cars: list, blueprint: BluePrint):
    ...
    if blueprint.brand != None:
        url += f"/merk={blueprint.brand}"

    if blueprint.model != None:
        url += f"/model={blueprint.model}"

    url += f"/prijs={blueprint.min_price}-{blueprint.max_price}"

    if blueprint.max_mileage > 0:
        url += f"/kilometerstand={blueprint.min_mileage}-{blueprint.max_mileage}"

    if blueprint.min_first_registration > 0:
        url += f"/vanaf-bouwjaar={blueprint.min_first_registration}"

    if blueprint.max_first_registration > 0:
        url += f"/tot-en-met-bouwjaar={blueprint.max_first_registration}"

    if blueprint.city != None and blueprint.max_distance_from_home > 0:
        url += f"/postcode-en-afstand={blueprint.city}-{blueprint.max_distance_from_home}km"
    ...
    logger.log_info("Scrape session started for ANWB")

    for i in range(0, 20):
        sleep(1)
        articles = main.find_elements_by_xpath(
            "//div[contains(@class, 'PONCHO-grid__item')]")
